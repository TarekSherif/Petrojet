...
$(function () {
        ...
        $('#jtableContainer').jtable({
            title: '<i class="fa fa-file-text  " style="color: orange;" aria-hidden="true"></i>	 @lang("messages.TUV")',
            paging: true,
            pageSize: 10,
            sorting: true,
            ...
            actions: {
				listAction:   '{{url("/")}}/api/ListOfTUVs?_token={{ csrf_token() }}',
                @if ($Permission["InsertData"])
                createAction: '{{url("/")}}/api/CreateTUV?_token={{ csrf_token() }}',
                @endif
                @if ($Permission["UpdateData"])
                updateAction: '{{url("/")}}/api/UpdateTUV?_token={{ csrf_token() }}',
				@endif
                @if ($Permission["DeleteData"])
                deleteAction: '{{url("/")}}/api/DeleteTUV?_token={{ csrf_token() }}'
                @endif

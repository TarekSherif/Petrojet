...
$(function () {
        ...
        $('#jtableContainer').jtable({
            title: '<i class="fa fa-file-text  " style="color: orange;" aria-hidden="true"></i>	 @lang("messages.jobTitle")',
            paging: true,
            pageSize: 10,
            sorting: true,
            ...
            actions: {
				listAction:   '{{url("/")}}/api/ListOfjobTitles?_token={{ csrf_token() }}',
               
                @if ($Permission["InsertData"])
                createAction: '{{url("/")}}/api/CreatejobTitle?_token={{ csrf_token() }}',
                @endif
                @if ($Permission["UpdateData"])
                updateAction: '{{url("/")}}/api/UpdatejobTitle?_token={{ csrf_token() }}',
				@endif
                @if ($Permission["DeleteData"])
                deleteAction: '{{url("/")}}/api/DeletejobTitle?_token={{ csrf_token() }}'
                @endif
